...
import { Search, MapPin, Phone, Car, Shield, DollarSign, ChevronLeft, ChevronRight, Menu, X, Star, Settings, Plus, Trash2, ArrowRight, Check, Calendar, Gauge, MessageCircle, Quote, Upload, User, Zap, CreditCard, Truck, Share2, Download, Flame } from 'lucide-react';
...
// רשימת סוכנים ל-CRM
const AGENTS = ["לא משויך", "יוסי", "דני", "אבי", "מיכל"];
...
const CAR_COLORS_MAP = {
  'לבן': '#FFFFFF', 'פנינה': '#FDEBD0', 'לבן פנינה': '#FDEBD0', 'שחור': '#111111',
  'שחור מטאלי': '#1A1A1A', 'כסף': '#C0C0C0', 'כסוף': '#C0C0C0', 'אפור': '#808080',
  'אפור עכבר': '#4B4B4B', 'אפור מטאלי': '#8A8D8F', 'בטון': '#7b7b7b', 'אפור בטון': '#7b7b7b',
  'אדום': '#CC0000', 'כחול': '#003399', 'כחול כהה': '#000033', 'תכלת': '#66B2FF',
  'ירוק': '#004d00', 'חום': '#5C4033', 'ברונזה': '#CD7F32', 'זהב': '#FFD700',
  'שמפניה': '#F7E7CE', 'בורדו': '#800000', 'צהוב': '#FFCC00', 'כתום': '#FF6600', 'סגול': '#4B0082',
};
...
const RevealOnScroll = ({ children, className = '', delay = 0, animation = 'fade-up' }) => {
  ...
  const base = "transition-all duration-[1200ms] ease-[cubic-bezier(0.25,1,0.5,1)] w-full"; 
  const hidden = {
    'fade-up': 'opacity-0 translate-y-16',
    'fade-in': 'opacity-0',
    'scale-up': 'opacity-0 scale-[0.92] translate-y-10',
    'slide-right': 'opacity-0 translate-x-12',
    'slide-left': 'opacity-0 -translate-x-12'
  }[animation] || 'opacity-0 translate-y-16';
  const visible = "opacity-100 translate-y-0 translate-x-0 scale-100";

  return (
    <div ref={ref} className={`${base} ${isVisible ? visible : hidden} ${className}`} style={{ transitionDelay: `${delay}ms` }}>
      {children}
    </div>
  );
};
...
const CarDealershipApp = () => {
  ...
  const EMPTY_CAR = { make:'',model:'',subModel:'',year:'',condition:'משומש',engineType:'בנזין',mileage:'',owners:'',engineCapacity:'',price:'',monthlyPayment:'',listPrice:'',showListPrice:false,images:[],type:'משפחתי', showOnHome: false, officialWarranty: false, colors: '', trims: [], status: 'זמין' };
  ...
  const [a11ySettings, setA11ySettings] = useState({ highContrast: false, largeText: false, highlightLinks: false, stopAnimations: false });

  const toggleA11y = (key) => setA11ySettings(prev => ({ ...prev, [key]: !prev[key] }));
  ...
  useEffect(() => {
    ...
    if (a11ySettings.stopAnimations) document.body.classList.add('a11y-stop-animations'); else document.body.classList.remove('a11y-stop-animations');
    if (a11ySettings.highlightLinks) document.body.classList.add('a11y-highlight-links'); else document.body.classList.remove('a11y-highlight-links');
  }, [a11ySettings]);
  ...
  const fetchInventory = async () => {
    ...
    try {
      const response = await fetch(`${mySupabaseUrl}/rest/v1/inventory?select=*&order=createdAt.desc`, { method: 'GET', headers: supabaseHeaders });
  ...
  const fetchLeads = async () => {
    ...
    try {
      const response = await fetch(`${mySupabaseUrl}/rest/v1/leads?select=*&order=created_at.desc`, { method: 'GET', headers: supabaseHeaders });
      if (!response.ok) throw new Error('שגיאה במשיכת לידים');
      const data = await response.json();
      setLeads(data || []);
    } catch (err) { console.error(err); } 
  ...
  useEffect(() => {
    if (isAdminOpen && adminTab === 'leads') fetchLeads();
  }, [isAdminOpen, adminTab]);

  const handleLeadUpdate = async (leadId, field, value) => {
    setLeads(prev => prev.map(l => l.id === leadId ? { ...l, [field]: value } : l));
    try {
      await fetch(`${mySupabaseUrl}/rest/v1/leads?id=eq.${leadId}`, {
        method: 'PATCH', headers: supabaseHeaders, body: JSON.stringify({ [field]: value })
      });
    } catch (err) { console.error(`שגיאה בעדכון ${field}:`, err); }
  };

  const handleExportCSV = () => {
    const headers = ['תאריך פנייה', 'שם הלקוח', 'טלפון', 'סוג ליד', 'פרטי רכב', 'סטטוס', 'סוכן', 'רמת עניין', 'ת. פולו-אפ', 'הערות'];
    const rows = leads.map(l => {
      return [
        new Date(l.created_at).toLocaleDateString('he-IL'),
        l.name || '', l.phone || '', l.lead_type || '', l.car_details || '', 
        l.status || 'חדש', l.agent || 'לא משויך', l.heat_level || '1', l.follow_up_date || '', l.notes || ''
    ...
    link.download = `leads_export_${new Date().toLocaleDateString('he-IL').replace(/\//g,'-')}.csv`;
    link.click();
  };
  ...
  const handleExportFacebookCatalog = () => {
    ...
    const rows = inventory.map(car => {
      ...
      const availability = car.status === 'נמכר' ? 'OUT_OF_STOCK' : 'AVAILABLE'; 
  ...
  const uploadImagesToStorage = async (files) => {
    ...
    for (const file of files) {
      ...
      const response = await fetch(`${mySupabaseUrl}/storage/v1/object/car-images/${fileName}`, {
        method: 'POST', headers: { 'apikey': mySupabaseKey, 'Authorization': `Bearer ${mySupabaseKey}`, 'Content-Type': 'image/jpeg' }, body: blob
      });
  ...
  const updateFacebookCatalog = async (currentCars) => {
    try {
      ...
      const response = await fetch(`${mySupabaseUrl}/storage/v1/object/catalog/facebook_catalog.csv`, {
        method: 'POST', headers: { ...supabaseHeaders, 'Content-Type': 'text/csv;charset=utf-8', 'x-upsert': 'true' }, body: csvContent
      });
    ...
    try {
      ...
      if (data.result && data.result.records.length > 0) {
        ...
        if (apiMake) {
          ...
          if (found) matchedMake = found; else if (apiMake.includes("ג'יפ") || apiMake.includes("JEEP")) matchedMake = "ג'יפ";
        }
        setNewCar(prev => ({
          ...prev, make: matchedMake || prev.make, model: carData.kinuy_mishari ? carData.kinuy_mishari.trim() : prev.model, subModel: carData.ramat_gimur ? carData.ramat_gimur.trim() : prev.subModel,
          year: carData.shnat_yitzur ? carData.shnat_yitzur.toString() : prev.year, engineCapacity: carData.nefach_manoa ? carData.nefach_manoa.toString() : prev.engineCapacity,
          engineType: (carData.sug_delek_nm && carData.sug_delek_nm.includes('חשמל')) ? 'חשמלי' : (carData.sug_delek_nm && (carData.sug_delek_nm.includes('היברידי') || carData.sug_delek_nm.includes('הייבריד'))) ? 'הייבריד' : (carData.sug_delek_nm && carData.sug_delek_nm.includes('דיזל')) ? 'דיזל' : (carData.sug_delek_nm && carData.sug_delek_nm.includes('בנזין')) ? 'בנזין' : prev.engineType,
        }));
        alert(`✅ הנתונים נמשכו בהצלחה!\nשים לב: יש להזין ידנית "יד" ו"קילומטראז'".`);
        setPlateNumber('');
      } else alert('❌ רכב לא נמצא במאגר משרד התחבורה.');
    } catch (error) { alert('אירעה שגיאה בחיבור למאגר.'); }
  };

  const handleImageSelection = (e, target = 'new') => {
    const files = Array.from(e.target.files);
    if (!files.length) return;
    if (files.length > 10) return alert('עד 10 תמונות');
  ...
  const handleAddCar = async (e) => {
    ...
    } catch (err) { setUploadStatus('error'); setTimeout(() => setUploadStatus('idle'), 4000); }
  };
  ...
  const handleEditSave = async (e) => {
    ...
    } catch (err) { setEditStatus('error'); setTimeout(() => setEditStatus('idle'), 4000); }
  };
  ...
  const handleDeleteCar = async (id) => {
    try {
      ...
      setInventory(p => { const newList = p.filter(c => c.id !== id); updateFacebookCatalog(newList); return newList; });
  ...
  const handleMarkAsSold = async (id) => {
    try {
      await fetch(`${mySupabaseUrl}/rest/v1/inventory?id=eq.${id}`, { method: 'PATCH', headers: supabaseHeaders, body: JSON.stringify({ status: 'נמכר' }) });
      setInventory(p => { const newList = p.map(c => c.id === id ? { ...c, status: 'נמכר' } : c); updateFacebookCatalog(newList); return newList; });
  ...
  const handleFinanceSearch = () => {
    ...
    const results = inventory.filter(c => c.status !== 'נמכר' && c.monthlyPayment && Number(c.monthlyPayment.toString().replace(/,/g,'')) <= max);
    setSearchResults(results); setSearchTitle('תוצאות חיפוש מימון'); setSearchSubtitle(`נמצאו ${results.length} רכבים בהחזר חודשי עד ₪${max.toLocaleString()}`); navigateTo('search');
  };

  const handleRegularSearch = () => {
    const results = inventory.filter(c => {
      if (c.status === 'נמכר') return false; 
    ...
    setSearchResults(results); setSearchTitle('תוצאות חיפוש'); setSearchSubtitle(`נמצאו ${results.length} רכבים התואמים לחיפוש שלך.`); navigateTo('search');
  };

  const navigateTo = (view, hash = null) => {
    setCurrentView(view); setSelectedCar(null); setIsMenuOpen(false); window.history.pushState({}, '', window.location.pathname);
    if (hash) setTimeout(() => document.getElementById(hash)?.scrollIntoView({ behavior: 'smooth' }), 100); else window.scrollTo(0, 0);
  };
  ...
  const handleShare = async (car, e) => {
    ...
    if (navigator.share) { try { await navigator.share({ title: 'אוטו מרקט - שיתוף רכב', text: shareText, url: shareUrl }); } catch (err) {} } 
        ...
        <RevealOnScroll key={car.id} animation="scale-up" delay={index * 150}>
          <div className="group bg-neutral-900 rounded-2xl overflow-hidden border border-neutral-800 hover:border-red-600/50 transition-all duration-500 hover:shadow-[0_10px_30px_rgba(220,38,38,0.15)] active:scale-[0.98]">
            ...
            <div className="p-4 text-right relative bg-neutral-900">
              <div className="mb-3">
                <h3 className="text-base font-bold text-white leading-tight truncate group-hover:text-red-500 transition-colors duration-300">{car.make} {car.model}</h3>
                ...
                <div className="mt-2 flex items-end justify-between flex-row-reverse">
                  <div>
                    {car.showListPrice && car.listPrice && (<span className="text-neutral-500 line-through text-xs block">מחירון ₪{car.listPrice}</span>)}
                    <p className="text-lg font-bold text-red-600 leading-tight">₪{car.price}</p>
                  </div>
                  {car.monthlyPayment && (<div className="bg-green-600/10 text-green-500 border border-green-500/30 text-xs font-bold px-2.5 py-1.5 rounded-lg inline-block shadow-sm">מ-₪{car.monthlyPayment}/לחודש</div>)}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-x-2 gap-y-1.5 mb-4 pt-3 border-t border-neutral-800/50 text-xs text-neutral-300">
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4 mb-6 space-y-3">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <select value={filterMake} onChange={e=>setFilterMake(e.target.value)} className={SELECT_CLASS}>
                  <option value="">כל היצרנים</option>{uniqueMakes.map(m=><option key={m} value={m}>{m}</option>)}
                </select>
                <select value={filterType} onChange={e=>setFilterType(e.target.value)} className={SELECT_CLASS}>
                  <option value="">כל הקטגוריות</option>{['משפחתי','יוקרה','ספורט','גיפ','7 מקומות','מיני','מנהלים'].map(o=><option key={o} value={o}>{o}</option>)}
                </select>
                <select value={filterEngine} onChange={e=>setFilterEngine(e.target.value)} className={SELECT_CLASS}>
                  <option value="">כל סוגי הנעה</option>{['בנזין','הייבריד','חשמלי','דיזל','פלאג אין הייבריד'].map(o=><option key={o} value={o}>{o}</option>)}
                </select>
                <button onClick={()=>{setFilterMake('');setFilterType('');setFilterEngine('');setFilterMaxPrice(1500000);}} className="bg-neutral-800 hover:bg-red-600 text-neutral-300 hover:text-white rounded-xl px-4 py-3.5 text-sm font-bold transition-colors touch-manipulation">נקה סינון</button>
              </div>
  ...
  const CarDetailsPage = ({ car, onBack, onOpenDigitalOrder }) => {
    ...
    const colorList = car.colors 
      ? car.colors.split(',').map(c => {
          const parts = c.trim().split(':'); const name = parts[0].trim(); const price = parts[1] ? parseInt(parts[1].replace(/\D/g, '')) || 0 : 0; return { name, price };
        }).filter(c => c.name) 
      : [];
    ...
    const getCurrentPrice = () => {
      ...
      if (selectedTrim && selectedTrim.price) finalPrice = parseInt(selectedTrim.price.toString().replace(/\D/g, '')) || finalPrice;
      if (selectedColor && selectedColor.price > 0) finalPrice += selectedColor.price;
      return finalPrice;
    };
    ...
    const handleLeadSubmit = async (e) => {
      ...
      try { await fetch(`${mySupabaseUrl}/rest/v1/leads`, { method: 'POST', headers: supabaseHeaders, body: JSON.stringify({ name: leadName, phone: leadPhone, lead_type: type, car_details: carDetailsString }) }); } catch (err) {}
            ...
            <div className="lg:col-span-2 space-y-4">
              <RevealOnScroll animation="fade-up">
                <div className="bg-neutral-900 rounded-2xl overflow-hidden border border-neutral-800 relative" style={{height:'min(65vw, 420px)'}}>
                  ...
                  {imgs.length > 1 && (
                    <>
                      <button onClick={() => setActiveImg(i => (i - 1 + imgs.length) % imgs.length)} className="absolute left-2 top-1/2 -translate-y-1/2 z-10 bg-black/50 backdrop-blur text-white p-2 rounded-full border border-white/10 active:bg-red-600 transition-colors"><ChevronLeft className="w-4 h-4" /></button>
                      <button onClick={() => setActiveImg(i => (i + 1) % imgs.length)} className="absolute right-2 top-1/2 -translate-y-1/2 z-10 bg-black/50 backdrop-blur text-white p-2 rounded-full border border-white/10 active:bg-red-600 transition-colors"><ChevronRight className="w-4 h-4" /></button>
              ...
              {imgs.length > 1 && (
                <div className="flex gap-2 overflow-x-auto pb-1 flex-row-reverse scrollbar-hide snap-x snap-mandatory">
                  {imgs.map((img,i) => (
                    <div key={i} onClick={()=>setActiveImg(i)} className={`shrink-0 snap-start rounded-lg overflow-hidden cursor-pointer border-2 transition-all touch-manipulation ${activeImg===i?'border-red-600':'border-transparent opacity-50'}`} style={{width:'72px',height:'50px'}}><img src={img} className="w-full h-full object-cover" alt="" /></div>
                  ))}
                </div>
              )}
                ...
                <RevealOnScroll animation="fade-up" delay={150}>
                  <div className="bg-neutral-900 p-4 rounded-2xl border border-neutral-800 text-right">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <div onClick={() => setSelectedTrim(null)} className={`border rounded-xl p-3 cursor-pointer transition-all ${!selectedTrim ? 'bg-red-600/10 border-red-500' : 'bg-neutral-950 border-neutral-800'}`}>
                        <p className="font-bold text-white text-sm">דגם בסיס</p><p className="text-xs text-red-500 font-bold mt-1">₪{getBasePrice().toLocaleString()}</p>
                      </div>
                      {car.trims.map((trim, idx) => (
                        <div key={idx} onClick={() => setSelectedTrim(trim)} className={`border rounded-xl p-3 cursor-pointer transition-all ${selectedTrim?.name === trim.name ? 'bg-red-600/10 border-red-500' : 'bg-neutral-950 border-neutral-800'}`}>
                          <p className="font-bold text-white text-sm">{trim.name}</p><p className="text-xs text-red-500 font-bold mt-1">₪{(parseInt(trim.price)||0).toLocaleString()}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-neutral-900 p-4 rounded-2xl border border-neutral-800 text-right">
                    ...
                    <div className="flex flex-wrap justify-center gap-3 flex-row-reverse">
                      {colorList.map((colorObj, idx) => {
                        ...
                        return (
                          <button key={idx} type="button" onClick={() => setSelectedColor(colorObj)} className={`flex items-center gap-2.5 px-3 py-2 rounded-xl transition-all border touch-manipulation flex-row-reverse ${isSelected ? 'bg-neutral-800 border-red-500 shadow-[0_0_12px_rgba(220,38,38,0.3)] text-white scale-105' : 'bg-neutral-950 text-neutral-400 border-neutral-800 hover:border-neutral-600 hover:text-neutral-200'}`}>
                            <span className={`block w-6 h-6 rounded-full shadow-inner flex-shrink-0 ${hexCode === '#FFFFFF' || hexCode === '#FDEBD0' ? 'border border-neutral-300' : 'border border-black/30'}`} style={{ backgroundColor: hexCode || 'transparent', backgroundImage: !hexCode ? 'linear-gradient(45deg, #ef4444, #3b82f6, #22c55e)' : 'none' }}/>
                            <div className="text-right">
                              <span className="block text-sm font-medium leading-none mt-1">{colorObj.name}</span>
                              {colorObj.price > 0 && (<span className="block text-xs text-red-500 font-bold mt-1.5 leading-none">+₪{colorObj.price.toLocaleString()}</span>)}
                            </div>
                          </button>
                        );
                      })}
              ...
              <RevealOnScroll animation="fade-up" delay={250}>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                  {[ { icon:<Calendar className="w-5 h-5 text-red-600"/>, label:'שנת ייצור', val:car.year }, { icon:<Settings className="w-5 h-5 text-red-600"/>, label:'סוג הנעה', val:car.engineType }, { icon:<Gauge className="w-5 h-5 text-red-600"/>, label:"קילומטראז'", val:`${car.mileage} km` }, { icon:<Car className="w-5 h-5 text-red-600"/>, label:'קטגוריה', val:car.type }, ].map((s,i) => (
                    <div key={i} className="bg-neutral-900 p-3 sm:p-4 rounded-xl border border-neutral-800 text-center"><div className="flex justify-center mb-1.5">{s.icon}</div><p className="text-neutral-400 text-xs mb-0.5">{s.label}</p><p className="text-white font-bold text-sm">{s.val}</p></div>
                  ))}
                </div>
              </RevealOnScroll>
                ...
                <div className="bg-neutral-900 p-4 md:p-8 rounded-2xl border border-neutral-800">
                  ...
                  <ul className="space-y-2.5">
                    {["אחריות מלאה בבדיקה על מנוע גיר ושלדת הרכב","100% מימון בתנאים הטובים ביותר.","אפשרות לטרייד-אין עתידי.","בדיקה קפדנית לפני מסירה.","ריביות מהנמוכות במשק"].map((t,i) => (
                      <li key={i} className="flex items-start gap-2.5 flex-row-reverse"><div className="mt-0.5 bg-red-600/20 p-1 rounded-full shrink-0"><Check className="w-3.5 h-3.5 text-red-500"/></div><span className="text-neutral-300 text-sm">{t}</span></li>
                    ))}
                  </ul>
                </div>
              </RevealOnScroll>
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans" dir="rtl">
      ...
      <nav className="fixed w-full z-50 bg-neutral-950/95 backdrop-blur-md border-b border-neutral-800 shadow-[0_2px_24px_rgba(0,0,0,0.6)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center" style={{height:'clamp(64px, 14vw, 112px)'}}>
            ...
            <button onClick={()=>setIsMenuOpen(!isMenuOpen)} className="lg:hidden text-neutral-300 hover:text-white p-2 rounded-xl bg-neutral-900/60 border border-neutral-800 touch-manipulation" style={{minHeight:'44px'}}>{isMenuOpen ? <X className="w-5 h-5"/> : <Menu className="w-5 h-5"/>}</button>
          </div>
        </div>
        {isMenuOpen && (
          <div className="lg:hidden bg-neutral-900 border-t border-neutral-800 shadow-2xl">
            {[ { label:'ראשי', action:()=>navigateTo('home') }, { label:'רכבים חדשים', action:()=>navigateTo('new') }, { label:'רכבים משומשים', action:()=>navigateTo('used') }, { label:'אודותינו', action:()=>navigateTo('home','about') } ].map((item,i) => (
              <button key={i} onClick={item.action} className="flex w-full items-center text-right px-5 py-4 text-neutral-300 hover:text-red-500 border-b border-neutral-800/60 text-base font-medium touch-manipulation"><ChevronLeft className="w-4 h-4 text-neutral-600 mr-auto"/>{item.label}</button>
            ))}
            <button onClick={()=>{setIsTradeInOpen(true);setIsMenuOpen(false);}} className="flex w-full items-center text-right px-5 py-4 text-red-500 font-bold border-b border-neutral-800/60 text-base touch-manipulation"><ChevronLeft className="w-4 h-4 text-red-800 mr-auto"/>הצעת טרייד אין</button>
            <div className="p-4 modal-safe-bottom"><button onClick={()=>navigateTo('home','contact')} className="block w-full text-center py-3.5 bg-red-600 text-white font-bold rounded-2xl text-base touch-manipulation">צור קשר</button></div>
          </div>
        )}
      </nav>
      ...
      {/* ──────── VIEWS ──────── */}
      {selectedCar ? (
        <CarDetailsPage car={selectedCar} onBack={() => { setSelectedCar(null); window.history.pushState({}, '', window.location.pathname); }} onOpenDigitalOrder={(car)=>{setDigitalOrderCar(car);setIsDigitalOrderOpen(true);}} />
      ) : currentView==='new' ? (
        <GenericInventoryPage cars={inventory.filter(c=>c.condition==='חדש' && c.status !== 'נמכר')} title={<>מלאי <span className="text-red-600">רכבים חדשים</span></>} subtitle={`${inventory.filter(c=>c.condition==='חדש' && c.status !== 'נמכר').length} רכבים — כל רכב עבר בדיקה קפדנית.`} />
      ) : currentView==='used' ? (
        <GenericInventoryPage cars={inventory.filter(c=>c.condition==='משומש' && c.status !== 'נמכר')} title={<>מלאי <span className="text-red-600">רכבים משומשים</span></>} subtitle={`${inventory.filter(c=>c.condition==='משומש' && c.status !== 'נמכר').length} רכבים — כל רכב עבר בדיקה קפדנית.`} />
      ) : currentView==='search' ? (
        ...
        <>
          ...
          <div className="relative flex items-center justify-center overflow-hidden" style={{paddingTop:'clamp(64px,14vw,112px)',minHeight:'100svh'}}>
            ...
            <div className="relative z-10 w-full max-w-5xl mx-auto px-4 py-8 md:py-10">
              <RevealOnScroll animation="fade-up" delay={100}>
                <h1 className="font-bold text-white text-center drop-shadow-lg leading-tight mb-3" style={{fontSize:'clamp(1.75rem, 7vw, 4.5rem)'}}>המסע שלך לרכב הבא<br/><span className="text-red-600">מתחיל כאן.</span></h1>
              </RevealOnScroll>
              
              <RevealOnScroll animation="fade-up" delay={250}>
                <p className="text-neutral-300 mb-6 max-w-2xl mx-auto text-center" style={{fontSize:'clamp(0.875rem, 2.5vw, 1.25rem)'}}>אוטו מרקט מתמחה בכל סוגי הרכבים. פתרונות מימון וטרייד-אין מותאמים אישית.</p>
              </RevealOnScroll>
                  ...
                  <div className="p-4 md:p-6">
                    {searchTab==='regular' && (
                      <div className="space-y-3">
                        ...
                        <button onClick={handleRegularSearch} className="w-full bg-red-600 text-white rounded-xl py-4 font-bold flex items-center justify-center gap-2 hover:bg-red-500 active:bg-red-700 transition-colors text-base flex-row-reverse touch-manipulation">חפש רכבים <Search className="w-5 h-5"/></button>
                      </div>
                    )}
                    {searchTab==='finance' && (
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <RevealOnScroll animation="fade-up">
                <div className="flex justify-between items-center mb-6">
                  <div className="text-right"><h2 className="font-bold" style={{fontSize:'clamp(1.25rem,5vw,2.5rem)'}}>רכבים <span className="text-red-600">חדשים</span></h2><p className="text-neutral-400 text-sm mt-1">מבחר הרכבים החדשים באולם שלנו.</p></div>
                  <button onClick={()=>navigateTo('new')} className="hidden md:flex items-center bg-white text-red-600 hover:bg-neutral-200 font-bold px-6 py-2.5 rounded-full gap-2 whitespace-nowrap text-sm transition-colors shadow-md min-h-0">כל המלאי ({inventory.filter(c=>c.condition==='חדש' && c.status !== 'נמכר').length}) <ChevronRight className="w-4 h-4 rotate-180"/></button>
                </div>
              </RevealOnScroll>
              <CarGrid cars={inventory.filter(c=>c.condition==='חדש' && c.showOnHome && c.status !== 'נמכר').slice(0,6)} />
              <div className="mt-5 md:hidden"><button onClick={()=>navigateTo('new')} className="w-full bg-white text-red-600 font-bold py-4 rounded-2xl flex items-center justify-center gap-2 text-sm touch-manipulation shadow-md">כל המלאי ({inventory.filter(c=>c.condition==='חדש' && c.status !== 'נמכר').length}) <ChevronRight className="w-4 h-4 rotate-180"/></button></div>
            </div>
          </section>
          ...
          <section className="py-12 md:py-24 bg-neutral-950">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <RevealOnScroll animation="fade-up">
                <div className="flex justify-between items-center mb-6">
                  <div className="text-right"><h2 className="font-bold" style={{fontSize:'clamp(1.25rem,5vw,2.5rem)'}}>רכבים <span className="text-red-600">משומשים</span></h2><p className="text-neutral-400 text-sm mt-1">רכבי יד שנייה מעולים לאחר בדיקה קפדנית.</p></div>
                  <button onClick={()=>navigateTo('used')} className="hidden md:flex items-center bg-white text-red-600 hover:bg-neutral-200 font-bold px-6 py-2.5 rounded-full gap-2 whitespace-nowrap text-sm transition-colors shadow-md min-h-0">כל המלאי ({inventory.filter(c=>c.condition==='משומש' && c.status !== 'נמכר').length}) <ChevronRight className="w-4 h-4 rotate-180"/></button>
                </div>
              </RevealOnScroll>
              <CarGrid cars={inventory.filter(c=>c.condition==='משומש' && c.showOnHome && c.status !== 'נמכר').slice(0,6)} />
              <div className="mt-5 md:hidden"><button onClick={()=>navigateTo('used')} className="w-full bg-white text-red-600 font-bold py-4 rounded-2xl flex items-center justify-center gap-2 text-sm touch-manipulation shadow-md">כל המלאי ({inventory.filter(c=>c.condition==='משומש' && c.status !== 'נמכר').length}) <ChevronRight className="w-4 h-4 rotate-180"/></button></div>
            </div>
          </section>
          ...
          <section className="py-12 md:py-24 bg-neutral-900 border-y border-neutral-800">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <RevealOnScroll animation="fade-up">
                <div className="text-center mb-8 md:mb-16"><h2 className="font-bold mb-3" style={{fontSize:'clamp(1.25rem,5vw,2.5rem)'}}>המעטפת <span className="text-red-600">המושלמת</span></h2><p className="text-neutral-400 max-w-2xl mx-auto text-sm md:text-base">חווית רכישה מלאה, מימון וטרייד-אין תחת קורת גג אחת.</p></div>
              </RevealOnScroll>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-8">
                {[ { icon:<DollarSign className="w-7 h-7"/>, title:'פתרונות מימון', text:'מימון אטרקטיבי עד 100% מול הגופים המובילים בענף.' }, { icon:<Car className="w-7 h-7"/>, title:'טרייד אין הוגן', text:'שקיפות מלאה ומחירון הוגן לרכב הישן שלך.' }, { icon:<Shield className="w-7 h-7"/>, title:'אחריות ובדיקה', text:'כל רכב עובר בדיקה מקיפה לשקט הנפשי שלך.' } ].map((s,i) => (
                  <RevealOnScroll key={i} animation="fade-up" delay={i * 200}>
                    <div className="bg-neutral-950 p-6 md:p-8 rounded-2xl border border-neutral-800 text-center hover:-translate-y-2 hover:border-red-600/30 hover:shadow-[0_15px_40px_rgba(220,38,38,0.1)] transition-all duration-500 cursor-default">
          ...
          <section id="about" className="py-12 md:py-24 bg-neutral-950 overflow-hidden">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex flex-col md:flex-row gap-8 md:gap-12 items-center md:flex-row-reverse">
                <RevealOnScroll animation="slide-left" className="w-full md:w-1/2 text-right">
                  ...
                  <ul className="space-y-3 mb-6">{[{b:'מבחר מוקפד:',t:'כל רכב נבחר בקפידה ועובר בדיקות קפדניות.'},{b:'שקיפות מלאה:',t:'כל ההיסטוריה של הרכב, ללא הפתעות.'}].map((item,i)=>(<li key={i} className="flex items-start gap-3 flex-row-reverse"><div className="mt-0.5 bg-red-600/20 p-1 rounded-full shrink-0"><Check className="w-3.5 h-3.5 text-red-500"/></div><span className="text-neutral-300 text-sm"><strong>{item.b}</strong> {item.t}</span></li>))}</ul>
                ...
                <RevealOnScroll animation="slide-right" className="w-full md:w-1/2 relative">
                  <div className="absolute inset-0 bg-red-600/20 rounded-3xl blur-3xl transform scale-95" />
                  <img src="https://i.postimg.cc/KjMP5Mc5/E7704582-D1FB-45CC-B594-D8641EFCB546.jpg" alt="אולם תצוגה" className="relative z-10 rounded-3xl shadow-2xl border border-neutral-800 w-full hover:scale-[1.02] transition-transform duration-700"/>
                </RevealOnScroll>
              </div>
            </div>
          </section>
          ...
          <section className="py-16 md:py-24 relative overflow-hidden bg-neutral-950 border-y border-red-900/30">
            <div className="absolute inset-0 bg-gradient-to-br from-red-950 via-neutral-950 to-black" /><div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(rgba(255,255,255,0.2) 1px, transparent 1px)', backgroundSize: '24px 24px' }} /><div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(220,38,38,0.25)_0,transparent_60%)]" />
            <RevealOnScroll animation="scale-up" className="max-w-4xl mx-auto px-4 relative z-10 text-center">
              ...
              <form className="flex flex-col gap-4 max-w-lg mx-auto" onSubmit={async(e)=>{ e.preventDefault(); const name=e.target.elements[0].value, phone=e.target.elements[1].value; try { await fetch(`${mySupabaseUrl}/rest/v1/leads`,{method:'POST',headers:supabaseHeaders,body:JSON.stringify({name,phone,lead_type:'פנייה כללית',car_details:'אין'})}); } catch(err){} window.open(`https://wa.me/972526441855?text=${encodeURIComponent(`שלום, הגעתי מהאתר.\nשם: ${name}\nטלפון: ${phone}`)}`, '_blank'); }}>
      ...
      <footer id="contact" className="bg-neutral-950 pt-12 md:pt-24 pb-10 border-t border-neutral-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-center text-center mb-10"><img src="/logo.png" alt="אוטו מרקט" className="h-14 md:h-20 w-auto object-contain mb-5"/><p className="text-neutral-400 max-w-xl text-sm md:text-lg">חווית רכישה, מימון וטרייד-אין תחת קורת גג אחת.</p></div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-10 border-t border-neutral-800/50 pt-10">
            <div className="text-right">
              ...
              <ul className="space-y-4 text-neutral-400">
                <li className="flex items-center gap-3 flex-row-reverse"><div className="bg-neutral-900 p-2.5 rounded-xl text-red-600"><Phone className="w-5 h-5"/></div><div><p className="text-xs text-neutral-500">טלפון</p><a href="tel:052-644-1855" className="text-white font-bold hover:text-red-500" dir="ltr">052-644-1855</a></div></li>
                <li className="flex items-center gap-3 flex-row-reverse"><div className="bg-neutral-900 p-2.5 rounded-xl text-red-600"><MapPin className="w-5 h-5"/></div><div><p className="text-xs text-neutral-500">כתובת</p><span className="text-white font-bold text-sm">פרנץ אופנהיימר 2, נתניה</span></div></li>
              </ul>
            </div>
            <div className="text-right sm:text-center">
              <h4 className="text-white font-bold mb-5 text-lg">ניווט מהיר</h4>
              <ul className="space-y-3 text-neutral-400 text-sm">{[{l:'רכבים חדשים',a:()=>navigateTo('new')},{l:'רכבים משומשים',a:()=>navigateTo('used')},{l:'טרייד-אין',a:()=>setIsTradeInOpen(true)},{l:'אודותינו',a:()=>navigateTo('home','about')}].map((item,i)=>(<li key={i}><button onClick={item.a} className="hover:text-red-600 transition-colors flex items-center gap-1.5 flex-row-reverse sm:mx-auto min-h-0 touch-manipulation"><ChevronRight className="w-3.5 h-3.5 text-red-600 rotate-180"/>{item.l}</button></li>))}</ul>
            </div>
            <div className="text-right">
              <h4 className="text-white font-bold mb-5 text-lg">שעות פעילות</h4>
              <ul className="space-y-2 text-neutral-400 text-sm">{[{d:"א' - ה'",h:'09:00 - 18:00'},{d:'שישי',h:'09:00 - 13:00'},{d:'שבת',h:null}].map((s,i)=>(<li key={i} className="flex justify-between border-b border-neutral-800/50 pb-2 flex-row-reverse"><span className="text-white">{s.d}</span>{s.h ? <span>{s.h}</span> : <span className="text-red-500">סגור</span>}</li>))}</ul>
            </div>
          </div>
          <div className="border-t border-neutral-900 pt-6 flex flex-col sm:flex-row justify-between items-center gap-3 flex-row-reverse text-sm text-neutral-500"><p>© {new Date().getFullYear()} Auto Market. כל הזכויות שמורות.</p><div className="flex gap-5 flex-row-reverse"><button onClick={(e) => { e.preventDefault(); setIsTermsOpen(true); }} className="hover:text-white transition-colors">תקנון האתר</button><button onClick={(e) => { e.preventDefault(); setIsAccessibilityOpen(true); }} className="hover:text-white transition-colors">הצהרת נגישות</button></div></div>
        </div>
      </footer>

      {/* ──── FABs ──── */}
      <a href="https://wa.me/972526441855?text=שלום,%20הגעתי%20מהאתר" target="_blank" rel="noopener noreferrer" className="fixed z-40 bg-[#25D366] text-white rounded-full shadow-[0_4px_20px_rgba(37,211,102,0.4)] hover:scale-110 active:scale-95 transition-all touch-manipulation" style={{bottom:'max(1.5rem,env(safe-area-inset-bottom,1.5rem))',right:'clamp(1rem,4vw,1.5rem)',padding:'clamp(0.75rem,2.5vw,1rem)'}}><MessageCircle style={{width:'clamp(1.5rem,6vw,2rem)',height:'clamp(1.5rem,6vw,2rem)'}}/></a>
      <div className="fixed z-50 flex flex-col items-start gap-3" style={{bottom: 'max(1.5rem,env(safe-area-inset-bottom,1.5rem))', left: 'clamp(1rem,4vw,1.5rem)'}}>
        {isA11yMenuOpen && (
          <div className="bg-neutral-900 border border-neutral-700 rounded-2xl p-4 shadow-[0_0_20px_rgba(0,0,0,0.5)] w-56 text-right flex flex-col gap-2 mb-1 touch-manipulation">
            <div className="flex justify-between items-center border-b border-neutral-800 pb-2 mb-1 flex-row-reverse"><button onClick={() => setIsA11yMenuOpen(false)} className="text-neutral-500 hover:text-white p-1"><X className="w-4 h-4"/></button><h4 className="text-white font-bold text-sm">תפריט נגישות</h4></div>
            <button onClick={() => toggleA11y('largeText')} className={`text-sm py-2 px-3 rounded-lg font-medium transition-colors flex items-center justify-between flex-row-reverse ${a11ySettings.largeText ? 'bg-blue-600 text-white' : 'bg-neutral-800 text-neutral-300 hover:bg-neutral-700'}`}><span className="text-lg leading-none font-bold">Aa</span> הגדלת טקסט</button>
            <button onClick={() => toggleA11y('highContrast')} className={`text-sm py-2 px-3 rounded-lg font-medium transition-colors flex items-center justify-between flex-row-reverse ${a11ySettings.highContrast ? 'bg-blue-600 text-white' : 'bg-neutral-800 text-neutral-300 hover:bg-neutral-700'}`}><span className="text-lg leading-none">◐</span> ניגודיות גבוהה</button>
            <button onClick={() => toggleA11y('highlightLinks')} className={`text-sm py-2 px-3 rounded-lg font-medium transition-colors flex items-center justify-between flex-row-reverse ${a11ySettings.highlightLinks ? 'bg-blue-600 text-white' : 'bg-neutral-800 text-neutral-300 hover:bg-neutral-700'}`}><span className="text-lg leading-none underline">U</span> הדגשת קישורים</button>
            <button onClick={() => toggleA11y('stopAnimations')} className={`text-sm py-2 px-3 rounded-lg font-medium transition-colors flex items-center justify-between flex-row-reverse ${a11ySettings.stopAnimations ? 'bg-blue-600 text-white' : 'bg-neutral-800 text-neutral-300 hover:bg-neutral-700'}`}><span className="text-lg leading-none">⏸</span> עצירת אנימציות</button>
            <button onClick={() => setIsAccessibilityOpen(true)} className="text-xs text-neutral-400 hover:text-white underline mt-2 text-center pt-2 border-t border-neutral-800">הצהרת נגישות מלאה</button>
          </div>
        )}
        <button onClick={() => setIsA11yMenuOpen(!isA11yMenuOpen)} className="bg-blue-600 text-white rounded-full shadow-[0_4px_20px_rgba(37,99,235,0.4)] hover:scale-110 active:scale-95 transition-all flex items-center justify-center border-2 border-white/10" style={{width:'clamp(3rem,6vw,3.5rem)', height:'clamp(3rem,6vw,3.5rem)'}} title="אפשרויות נגישות">
      ...
      {isTermsOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-3xl flex flex-col shadow-2xl" style={{maxHeight:'85svh'}}>
            <div className="flex justify-between items-center px-5 py-4 border-b border-neutral-800 bg-neutral-950 flex-row-reverse shrink-0"><button onClick={() => setIsTermsOpen(false)} className="bg-neutral-800 hover:bg-red-600 p-2 rounded-full text-neutral-400 hover:text-white transition-colors min-h-0 touch-manipulation"><X className="w-5 h-5"/></button><h2 className="text-xl font-bold text-white text-right">תקנון האתר</h2></div>
            <div className="p-6 overflow-y-auto text-right text-neutral-300 text-sm space-y-4" dir="rtl"><p>ברוכים הבאים לאתר "אוטו מרקט". השימוש באתר, בתכניו ובשירותים המוצעים בו, כפוף לתנאים המפורטים בתקנון זה. גלישה באתר, הזנת פרטים או ביצוע פעולה כלשהי מהווים הסכמה מוחלטת לתנאי התקנון. במידה ואינך מסכים לתנאי כלשהו, הנך מתבקש להימנע משימוש באתר.</p><h3 className="text-white font-bold text-base mt-4">1. מידע, מלאי והמחשה</h3><p>האתר נועד לספק מידע אודות מלאי הרכבים בסוכנות, אפשרויות מימון וטרייד-אין. כל המידע המוצג באתר, לרבות מפרטים טכניים, תוספות, צבעים ותמונות, הינו להמחשה בלבד. הנהלת האתר אינה מתחייבת כי רכב מסוים המוצג באתר זמין במלאי הפיזי בכל רגע נתון. במקרה של סתירה או אי-התאמה בין המפורט באתר לבין חוזה הרכישה הפיזי שייחתם בסוכנות, המידע בחוזה הרכישה הרשמי הוא הקובע והבלעדי.</p><h3 className="text-white font-bold text-base mt-4">2. מחירים וסימולטור מימון</h3><p>המחירים המוצגים באתר עשויים להשתנות מעת לעת וללא הודעה מוקדמת, ואינם כוללים אגרת רישוי, העברת בעלות או אגרות ממשלתיות נוספות (אלא אם צוין מפורשות אחרת). התוספות עבור צבעים, דגמים ואבזור נתונות לשינוי.<br/>סימולטור המימון מיועד לתת אומדן כללי ומשוער בלבד ואינו מהווה הצעה מחייבת. קבלת המימון, גובה הריבית, מספר התשלומים וההחזר החודשי הסופי כפופים לחלוטין לאישור הגוף המממן ולבדיקת נתוני האשראי של הלקוח. הנהלת האתר אינה צד לעסקת המימון ואינה נושאת באחריות לסירובו של גוף מממן לאשר את הבקשה.</p><h3 className="text-white font-bold text-base mt-4">3. הזמנה דיגיטלית ושריון רכב (מקדמה)</h3><p>תשלום המקדמה באמצעות האתר נועד למטרת "שריון" בלבד של הרכב הנבחר למשך זמן מוגבל. שריון זה אינו מהווה עסקת מכר סופית, והעסקה תושלם אך ורק לאחר חתימה על הסכם הזמנה/רכישה מלא פיזית בסוכנות והשלמת מלוא התמורה.<br/>מדיניות ביטולים: הלקוח רשאי לבטל את ההזמנה הדיגיטלית (שריון הרכב) בהתאם להוראות חוק הגנת הצרכן, התשמ"א-1981. במקרה של ביטול העסקה טרם חתימה על הסכם רכישה בסוכנות, המקדמה תוחזר ללקוח, בניכוי דמי ביטול בשיעור של 5% מסכום המקדמה או 100 ש"ח, הנמוך מביניהם, כקבוע בחוק.</p><h3 className="text-white font-bold text-base mt-4">4. כשירות המשתמש וחובת מסירת פרטים נכונים</h3><p>השימוש באתר מותר לבני 18 ומעלה, הכשירים משפטית לבצע פעולות משפטיות מחייבות, ובעלי כרטיס אשראי ישראלי תקף. המשתמש מתחייב למסור פרטים אישיים מדויקים ואמיתיים. מסירת פרטים כוזבים מהווה עבירה פלילית ותגרור נקיטת אמצעים משפטיים.</p><h3 className="text-white font-bold text-base mt-4">5. פרטיות, אבטחת מידע ודיוור ישיר</h3><p>אנו מכבדים את פרטיות הלקוחות. הפרטים המוזנים באתר לא יועברו לצד שלישי שאינו קשור לעסקה (כגון חברות מימון או סליקה), ללא הסכמת הלקוח. תשלום המקדמה מבוצע באמצעות חברת סליקה חיצונית ומאובטחת תחת התקנים המחמירים ביותר (PCI-DSS).<br/>בהשארת פרטים באתר, המשתמש נותן הסכמה מפורשת לקבלת דברי פרסומת ועדכונים, בהתאם לחוק התקשורת (חוק הספאם). המשתמש רשאי להסיר את עצמו בכל עת.</p><h3 className="text-white font-bold text-base mt-4">6. אחריות ובדיקת רכבים</h3><p>6.1 תנאי האחריות משתנים בין רכב לרכב ובהתאם להסכם המכירה הספציפי.<br/>6.2 על הלקוח מוטלת האחריות לבדוק את הרכב טרם הרכישה, לרבות באמצעות מכון בדיקה מטעמו.<br/>6.3 החברה אינה אחראית לבלאי סביר, שימוש קודם, תיקונים קודמים או כל פרט שלא הוצהר במפורש בהסכם המכירה.</p><h3 className="text-white font-bold text-base mt-4">7. קניין רוחני</h3><p>כל זכויות הקניין הרוחני באתר – לרבות העיצוב, קוד המקור, הטקסטים והתמונות – הינן רכושה הבלעדי של הנהלת "אוטו מרקט". אין להעתיק או להשתמש במידע כלשהו מן האתר ללא קבלת אישור מפורש ובכתב.</p><h3 className="text-white font-bold text-base mt-4">8. הגבלת אחריות</h3><p>הנהלת האתר אינה מתחייבת שהשירות באתר לא יופרע או יינתן ללא תקלות. האתר לא יישא באחריות לכל נזק שייגרם כתוצאה משימוש באתר.</p><h3 className="text-white font-bold text-base mt-4">9. ט.ל.ח (טעות לעולם חוזרת)</h3><p>אנו עושים מאמץ להציג מידע מדויק. עם זאת, ייתכנו טעויות אנוש בהזנת נתונים או מחירים. הנהלת האתר שומרת לעצמה את הזכות המלאה לבטל הזמנות ולתקן שגיאות אלו ולא תהיה מחויבת למחיר או מפרט שגוי שפורסם.</p><h3 className="text-white font-bold text-base mt-4">10. דין וסמכות שיפוט</h3><p>על תקנון זה ועל השימוש באתר יחולו דיני מדינת ישראל בלבד. סמכות השיפוט הבלעדית תהיה נתונה לבתי המשפט המוסמכים במחוז מרכז.</p></div>
            <div className="p-4 border-t border-neutral-800 bg-neutral-950 shrink-0"><button onClick={() => setIsTermsOpen(false)} className="w-full bg-neutral-800 hover:bg-neutral-700 text-white font-bold py-3 rounded-xl transition-colors">קראתי והבנתי, סגור חלון</button></div>
          </div>
        </div>
      )}
      ...
      {isAccessibilityOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-3xl flex flex-col shadow-2xl" style={{maxHeight:'85svh'}}>
            <div className="flex justify-between items-center px-5 py-4 border-b border-neutral-800 bg-neutral-950 flex-row-reverse shrink-0"><button onClick={() => setIsAccessibilityOpen(false)} className="bg-neutral-800 hover:bg-red-600 p-2 rounded-full text-neutral-400 hover:text-white transition-colors min-h-0 touch-manipulation"><X className="w-5 h-5"/></button><h2 className="text-xl font-bold text-white text-right">הצהרת נגישות</h2></div>
            <div className="p-6 overflow-y-auto text-right text-neutral-300 text-sm space-y-4" dir="rtl"><p>אנו ב"אוטו מרקט" רואים חשיבות עליונה בהנגשת האתר והשירותים שלנו לאנשים עם מוגבלויות, מתוך אמונה כי לכל אדם מגיעה הזכות לשוויון, כבוד, נוחות ועצמאות.</p><h3 className="text-white font-bold text-base mt-4">רמת הנגישות באתר</h3><p>האתר הונגש בהתאם לתקנות שוויון זכויות לאנשים עם מוגבלות (התאמות נגישות לשירות), התשע"ג-2013, ומותאם לדרישות התקן הישראלי (ת"י 5568) לרמת נגישות AA ולמסמך WCAG 2.0 הבינלאומי.</p><h3 className="text-white font-bold text-base mt-4">אמצעי הנגישות באתר</h3><ul className="list-disc list-inside space-y-2 text-neutral-400"><li><strong>תפריט נגישות:</strong> באתר מותקן תוסף נגישות מתקדם. לחיצה על סמל הנגישות תפתח את תפריט ההנגשה.</li><li><strong>הגדלת גופן:</strong> אפשרות להגדלת והקטנת הטקסט באתר לנוחות הקריאה.</li><li><strong>ניגודיות:</strong> אפשרות לשינוי ניגודיות הצבעים (רקע כהה, רקע בהיר, מונוכרום).</li><li><strong>ניווט מקלדת:</strong> האתר מותאם לניווט בעזרת מקשי המקלדת (Tab, Enter, חיצים).</li><li><strong>התאמה לקורא מסך:</strong> האתר כתוב בצורה סמנטית ומותאם לתוכנות קוראות מסך (כגון NVDA, JAWS).</li></ul><h3 className="text-white font-bold text-base mt-4">פרטי רכז הנגישות ופניות בנושא</h3><p>אנו ממשיכים במאמצים לשפר את נגישות האתר. אם נתקלתם בבעיית נגישות, נשמח לקבל פנייה ולטפל בה בהקדם.</p><ul className="space-y-1 text-neutral-400 mt-2"><li><strong>שם רכז הנגישות:</strong> מנהל האתר</li><li><strong>טלפון:</strong> 052-644-1855</li><li><strong>פנייה בוואטסאפ:</strong> באמצעות כפתור הוואטסאפ באתר.</li></ul></div>
            <div className="p-4 border-t border-neutral-800 bg-neutral-950 shrink-0"><button onClick={() => setIsAccessibilityOpen(false)} className="w-full bg-neutral-800 hover:bg-neutral-700 text-white font-bold py-3 rounded-xl transition-colors">סגור חלון</button></div>
          </div>
        </div>
      )}
      ...
      {isPasswordPromptOpen && (
        <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="bg-neutral-900 border border-neutral-800 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-sm p-6 text-right shadow-2xl modal-safe-bottom">
            <div className="flex justify-between items-center mb-5 flex-row-reverse"><h3 className="text-xl font-bold text-white">כניסה לניהול מלאי</h3><button onClick={()=>{setIsPasswordPromptOpen(false);setPasswordError('');setAdminPassword('');}} className="text-neutral-400 hover:text-white bg-neutral-800 p-2 rounded-full min-h-0 touch-manipulation"><X className="w-5 h-5"/></button></div>
        ...
        <div className="fixed inset-0 z-[120] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="bg-neutral-950 border border-neutral-800 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-2xl flex flex-col shadow-2xl" style={{maxHeight:'92svh'}}>
            <div className="bg-neutral-900 px-5 py-4 text-center border-b border-neutral-800 shrink-0 rounded-t-3xl sm:rounded-t-2xl relative">
              ...
              <h2 className="text-xl font-bold text-white">הצעת טרייד אין <span className="text-red-600">מהירה</span></h2><p className="text-neutral-400 text-sm mt-0.5">מלאו פרטים וקבלו הצעה אטרקטיבית.</p>
            </div>
            <div className="p-4 overflow-y-auto modal-safe-bottom">
              <form onSubmit={handleTradeInSubmit} className="grid grid-cols-2 gap-3 text-sm">
                {[ {label:'שם מלא',key:'name',type:'text',ph:"ישראל ישראלי"}, {label:'טלפון',key:'phone',type:'tel',ph:"052-1234567"}, {label:'יצרן',key:'make',type:'text',ph:"יונדאי"}, {label:'דגם',key:'model',type:'text',ph:"טוסון Elite"}, {label:'שנתון',key:'year',type:'number',ph:"2021"} ].map(f=>(
                  <div key={f.key} className={f.key==='name'||f.key==='phone'?'col-span-1':''}><label className="block text-neutral-400 mb-1 text-xs font-medium">{f.label}</label><input type={f.type} value={tradeData[f.key]} onChange={e=>setTradeData({...tradeData,[f.key]:e.target.value})} placeholder={f.ph} className={INPUT_CLASS} required/></div>
                ))}
                <div><label className="block text-neutral-400 mb-1 text-xs font-medium">סוג הנעה</label><select value={tradeData.engine} onChange={e=>setTradeData({...tradeData,engine:e.target.value})} className={SELECT_CLASS} required><option value="" disabled>בחר</option>{['בנזין','הייבריד','חשמלי','דיזל','פלאג אין הייבריד'].map(o=><option key={o} value={o}>{o}</option>)}</select></div>
                <div><label className="block text-neutral-400 mb-1 text-xs font-medium">קילומטר</label><input type="number" value={tradeData.km} onChange={e=>setTradeData({...tradeData,km:e.target.value})} placeholder="45000" className={INPUT_CLASS} required/></div>
                <div><label className="block text-neutral-400 mb-1 text-xs font-medium">בעלות מקורית</label><select value={tradeData.ownership} onChange={e=>setTradeData({...tradeData,ownership:e.target.value})} className={SELECT_CLASS} required><option value="" disabled>בחר</option>{['פרטית','ליסינג','השכרה','חברה'].map(o=><option key={o} value={o}>{o}</option>)}</select></div>
                <div><label className="block text-neutral-400 mb-1 text-xs font-medium">יד הרכב</label><input type="number" value={tradeData.owners} onChange={e=>setTradeData({...tradeData,owners:e.target.value})} placeholder="1" className={INPUT_CLASS} required/></div>
                <div className="col-span-2 mt-1"><button type="submit" className="w-full bg-red-600 hover:bg-red-500 active:bg-red-700 text-white font-bold py-4 rounded-xl transition-colors flex items-center justify-center gap-2 flex-row-reverse touch-manipulation">שלח בוואטסאפ <MessageCircle className="w-5 h-5"/></button></div>
              </form>
            </div>
          </div>
        </div>
              ...
              {adminTab === 'inventory' ? (
                <>
                  ...
                  <div className="bg-neutral-950 rounded-2xl border border-neutral-800 overflow-hidden">
                    <div className="flex items-center gap-2 flex-row-reverse px-4 py-3 border-b border-neutral-800 bg-neutral-900/60"><Plus className="w-4 h-4 text-red-600"/><h3 className="font-bold text-white text-sm">הוספת רכב חדש</h3></div>
                    <div className="p-4 border-b border-neutral-800 bg-neutral-900/30">
                      ...
                      <div className="flex gap-3 flex-row-reverse">
                        <input type="text" placeholder="הזן מספר רישוי (ללא מקפים)" value={plateNumber} onChange={e => setPlateNumber(e.target.value.replace(/\D/g, ''))} className="flex-1 bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-red-600 outline-none"/>
                        <button type="button" onClick={handleFetchByPlate} className="bg-neutral-800 hover:bg-red-600 text-white px-5 py-2.5 rounded-lg transition-colors font-bold text-sm touch-manipulation">חפש רכב</button>
                      </div>
                    </div>
                    <div className="p-4">
                      <form onSubmit={handleAddCar} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 text-sm">
                        <select required value={newCar.make} onChange={e=>setNewCar({...newCar,make:e.target.value})} className="bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-red-600 outline-none col-span-2 md:col-span-1"><option value="">יצרן *</option>{ISRAELI_CAR_MAKES.map(m=><option key={m} value={m}>{m}</option>)}</select>
                        {[ {ph:'דגם *',k:'model',t:'text'},{ph:'תת דגם *',k:'subModel',t:'text'},{ph:'שנתון *',k:'year',t:'number'}, {ph:"קילומטראז' *",k:'mileage',t:'text'},{ph:'יד הרכב *',k:'owners',t:'number'},{ph:'נפח מנוע סמ"ק *',k:'engineCapacity',t:'text'}, {ph:'מחיר ₪ *',k:'price',t:'text'},{ph:'החזר חודשי ₪',k:'monthlyPayment',t:'text'} ].map(f=>(
                        ...
                        <select value={newCar.condition} onChange={e=>setNewCar({...newCar,condition:e.target.value})} className="bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-red-600 outline-none"><option value="משומש">משומש</option><option value="חדש">חדש</option></select>
                        <select value={newCar.engineType} onChange={e=>setNewCar({...newCar,engineType:e.target.value})} className="bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-red-600 outline-none">{['בנזין','הייבריד','חשמלי','דיזל','פלאג אין הייבריד'].map(o=><option key={o} value={o}>{o}</option>)}</select>
                        <select value={newCar.type} onChange={e=>setNewCar({...newCar,type:e.target.value})} className="bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-red-600 outline-none">{['משפחתי','יוקרה','ספורט','גיפ',"7 מקומות","מיני","מנהלים"].map(o=><option key={o} value={o}>{o}</option>)}</select>
                        ...
                        <div className="col-span-2 md:col-span-3 lg:col-span-4 flex flex-wrap items-center gap-4 flex-row-reverse bg-neutral-900/50 p-3 rounded-xl border border-neutral-800">
                          <div className="flex-1 flex items-center gap-2 flex-row-reverse min-w-[200px]"><input type="text" placeholder="מחיר מחירון ₪" value={newCar.listPrice} onChange={e=>setNewCar({...newCar,listPrice:e.target.value})} disabled={!newCar.showListPrice} className="flex-1 bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2.5 text-white disabled:opacity-40 text-right outline-none"/><label className="flex items-center gap-1.5 text-xs text-neutral-400 cursor-pointer whitespace-nowrap flex-row-reverse min-h-0"><input type="checkbox" checked={newCar.showListPrice} onChange={e=>setNewCar({...newCar,showListPrice:e.target.checked})} className="accent-red-600 w-4 h-4"/>הצג מחירון</label></div>
                          <label className="flex items-center gap-2 text-sm text-white font-bold cursor-pointer whitespace-nowrap flex-row-reverse bg-red-600/20 px-4 py-2.5 rounded-lg border border-red-600/40 hover:bg-red-600/30 transition-colors min-h-0"><input type="checkbox" checked={newCar.showOnHome||false} onChange={e=>setNewCar({...newCar,showOnHome:e.target.checked})} className="accent-red-600 w-4 h-4"/>הצג בדף הבית</label>
                          <label className="flex items-center gap-2 text-sm text-white font-bold cursor-pointer whitespace-nowrap flex-row-reverse bg-blue-600/20 px-4 py-2.5 rounded-lg border border-blue-600/40 hover:bg-blue-600/30 transition-colors"><input type="checkbox" checked={newCar.officialWarranty||false} onChange={e=>setNewCar({...newCar,officialWarranty:e.target.checked})} className="accent-blue-600 w-4 h-4"/>אחריות יבואן</label>
                        </div>
                        <div className="col-span-2 md:col-span-3 lg:col-span-4 text-right"><label className="block text-xs text-neutral-400 mb-1.5">צבעים זמינים (מופרדים בפסיק. למשל: לבן, שחור, כסף)</label><input type="text" placeholder="הזן צבעים..." value={newCar.colors || ''} onChange={e=>setNewCar({...newCar, colors: e.target.value})} className="w-full bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-red-600 outline-none"/></div>
                        <div className="col-span-2 md:col-span-3 lg:col-span-4 bg-neutral-900/50 p-4 rounded-xl border border-neutral-800">
                          <div className="flex justify-between items-center mb-3 flex-row-reverse"><label className="text-sm font-bold text-white">רמות גימור / דגמים נוספים לרכב זה</label><button type="button" onClick={() => setNewCar({...newCar, trims: [...(newCar.trims || []), {name: '', price: ''}]})} className="text-xs bg-neutral-800 hover:bg-red-600 text-white px-3 py-2 rounded-lg transition-colors shadow-sm font-medium">+ הוסף דגם</button></div>
                        ...
                        <div className="col-span-2 md:col-span-3 lg:col-span-4 text-right"><label className="block text-xs text-neutral-400 mb-1.5">העלאת תמונות (עד 10)</label><input type="file" accept="image/*" multiple onChange={e=>handleImageSelection(e,'new')} className="w-full bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-2 text-white text-xs file:ml-2 file:py-1 file:px-3 file:rounded-full file:border-0 file:bg-red-600 file:text-white cursor-pointer"/>{selectedFiles.length>0&&<p className="text-xs text-green-500 mt-1">✓ {selectedFiles.length} תמונות נבחרו</p>}</div>
                        <button type="submit" disabled={uploadStatus==='loading'} className={`col-span-2 md:col-span-3 lg:col-span-4 font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 flex-row-reverse transition-all text-white touch-manipulation ${uploadStatus==='loading'?'bg-neutral-700 cursor-not-allowed':'bg-red-600 hover:bg-red-500'}`}>{uploadStatus==='loading'?<><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"/>מעלה...</>:<><Plus className="w-5 h-5"/>העלה רכב</>}</button>
                      </form>
                      ...
                      {uploadStatus==='error'&&<div className="mt-3 flex items-center gap-2 bg-red-500/10 border border-red-500/30 text-red-400 rounded-xl px-4 py-3 flex-row-reverse"><X className="w-5 h-5"/><span className="text-sm">שגיאה בשמירה</span></div>}
                    </div>
                  </div>
                            ...
                            <tr key={car.id} className="hover:bg-neutral-900/40">
                              <td className="px-3 py-3">
                                <div className="flex gap-1.5 justify-center">
                                  {car.status !== 'נמכר' && (
                                    <button onClick={()=>handleMarkAsSold(car.id)} className="p-1.5 bg-neutral-800 hover:bg-green-600 text-neutral-400 hover:text-white rounded-lg transition-colors min-h-0 touch-manipulation" title="סמן כנמכר">
                ...
                /* ──── CRM LEADS TABLE (ADVANCED) ──── */
                <div className="flex flex-col gap-4 text-right">
                  {/* דאשבורד סטטיסטיקות */}
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-4 flex flex-col items-center justify-center text-center shadow-lg">
                      <span className="text-neutral-400 text-xs font-bold mb-1">סה"כ לידים</span>
                      <span className="text-2xl font-black text-white">{leads.length}</span>
                    </div>
                    <div className="bg-red-600/10 border border-red-600/30 rounded-xl p-4 flex flex-col items-center justify-center text-center shadow-lg">
                      <span className="text-red-400 text-xs font-bold mb-1">לידים חדשים</span>
                      <span className="text-2xl font-black text-red-500">{leads.filter(l => !l.status || l.status === 'חדש').length}</span>
                    </div>
                    <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 flex flex-col items-center justify-center text-center shadow-lg">
                      <span className="text-yellow-400 text-xs font-bold mb-1">בטיפול / משא ומתן</span>
                      <span className="text-2xl font-black text-yellow-500">{leads.filter(l => l.status === 'בטיפול').length}</span>
                    </div>
                    <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4 flex flex-col items-center justify-center text-center shadow-lg">
                      <span className="text-green-400 text-xs font-bold mb-1">עסקאות שנסגרו</span>
                      <span className="text-2xl font-black text-green-500">{leads.filter(l => l.status === 'טופל').length}</span>
                    </div>
                  </div>

                  {/* שורת חיפוש ופעולות */}
                  <div className="flex flex-col md:flex-row justify-between items-center gap-3 bg-neutral-950 p-3 rounded-xl border border-neutral-800 flex-row-reverse">
                    <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto flex-row-reverse">
                      <div className="relative w-full md:w-64">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-500" />
                        <input 
                          type="text" placeholder="חיפוש חופשי..." value={crmSearch} onChange={(e) => setCrmSearch(e.target.value)}
                          className="w-full bg-neutral-900 border border-neutral-700 rounded-lg pl-10 pr-4 py-2 text-white text-right focus:border-blue-500 outline-none text-sm"
                        />
                      </div>
                      <select value={crmStatusFilter} onChange={(e) => setCrmStatusFilter(e.target.value)} className="w-full md:w-40 bg-neutral-900 border border-neutral-700 rounded-lg px-4 py-2 text-white text-right focus:border-blue-500 outline-none text-sm">
                        <option value="all">כל הסטטוסים</option>
                        <option value="חדש">חדש</option>
                        <option value="בטיפול">בטיפול</option>
                        <option value="טופל">נסגר (טופל)</option>
                        <option value="לא רלוונטי">לא רלוונטי</option>
                      </select>
                    </div>
                    <div className="flex items-center gap-2 flex-row-reverse w-full md:w-auto">
                      <button onClick={handleExportCSV} className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-green-600/20 hover:bg-green-600 border border-green-600/50 text-green-500 hover:text-white px-4 py-2 rounded-lg transition-colors text-sm font-medium flex-row-reverse">
                        <Download className="w-4 h-4" /> ייצוא לאקסל
                      </button>
                      <button onClick={fetchLeads} className="flex-1 md:flex-none bg-neutral-800 hover:bg-neutral-700 text-white px-4 py-2 rounded-lg transition-colors text-sm font-medium">רענן</button>
                    </div>
                  </div>
                  ...
                  <div className="bg-neutral-950 rounded-2xl border border-neutral-800 overflow-hidden">
                    <div className="overflow-x-auto">
                      {leadsLoading ? (
                        <div className="py-12 text-center text-neutral-500"><div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto mb-3"/>טוען לידים...</div>
                      ) : (
                        <table className="w-full text-right text-sm">
                          <thead className="bg-neutral-900/80 text-neutral-400 border-b border-neutral-800 text-xs">
                            <tr>
                              <th className="px-3 py-3 text-center">וואטסאפ</th>
                              <th className="px-3 py-3 text-center">רמת חום</th>
                              <th className="px-3 py-3">שיוך סוכן</th>
                              <th className="px-3 py-3">ת. פולו-אפ</th>
                              <th className="px-3 py-3">סטטוס</th>
                              <th className="px-3 py-3">הערות אישיות</th>
                              <th className="px-3 py-3">רכב נבחר / פרטים</th>
                              <th className="px-3 py-3">סוג ליד</th>
                              <th className="px-3 py-3">שם וטלפון</th>
                              <th className="px-3 py-3">תאריך</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-neutral-800/60">
                            {leads
                              .filter(lead => crmStatusFilter === 'all' || lead.status === crmStatusFilter || (!lead.status && crmStatusFilter === 'חדש'))
                              .filter(lead => lead.name?.includes(crmSearch) || lead.phone?.includes(crmSearch) || lead.car_details?.includes(crmSearch))
                              .map(lead => {
                                const waMessage = `היי ${lead.name?.split(' ')[0]}, שמחתי לשוחח איתך! בהמשך להתעניינותך ב${lead.car_details?.split('(')[0] || 'רכב'}, אשמח לעזור לך להתקדם.`;
                                return (
                                  <tr key={lead.id} className="hover:bg-neutral-900/40">
                                    <td className="px-3 py-3 text-center">
                                      <a href={`https://wa.me/972${lead.phone?.replace(/^0/, '')}?text=${encodeURIComponent(waMessage)}`} target="_blank" rel="noreferrer" className="p-2 bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366] hover:text-white rounded-lg transition-colors flex items-center justify-center gap-1 mx-auto min-h-0" style={{width:'32px'}} title="שלח הודעה"><MessageCircle className="w-4 h-4"/></a>
                                    </td>
                                    <td className="px-3 py-3">
                                      <select value={lead.heat_level || '1'} onChange={(e) => handleLeadUpdate(lead.id, 'heat_level', e.target.value)} className="bg-transparent border-none text-base outline-none cursor-pointer">
                                        <option value="1">🔥</option><option value="2">🔥🔥</option><option value="3">🔥🔥🔥</option>
                                      </select>
                                    </td>
                                    <td className="px-3 py-3">
                                      <select value={lead.agent || 'לא משויך'} onChange={(e) => handleLeadUpdate(lead.id, 'agent', e.target.value)} className="bg-neutral-900 border border-neutral-700 text-white text-xs rounded-lg px-2 py-1.5 focus:border-blue-500 outline-none">
                                        {AGENTS.map(agent => <option key={agent} value={agent}>{agent}</option>)}
                                      </select>
                                    </td>
                                    <td className="px-3 py-3">
                                      <input type="date" value={lead.follow_up_date || ''} onChange={(e) => handleLeadUpdate(lead.id, 'follow_up_date', e.target.value)} className="bg-neutral-900 border border-neutral-700 text-white text-xs rounded-lg px-2 py-1.5 focus:border-blue-500 outline-none" style={{colorScheme:'dark'}}/>
                                    </td>
                                    <td className="px-3 py-3">
                                      <select value={lead.status || 'חדש'} onChange={(e) => handleLeadUpdate(lead.id, 'status', e.target.value)} className={`bg-neutral-800 border border-neutral-700 text-xs rounded-lg px-2 py-1.5 focus:border-blue-500 outline-none font-bold appearance-none ${lead.status === 'טופל' ? 'text-green-500' : lead.status === 'לא רלוונטי' ? 'text-neutral-500' : lead.status === 'בטיפול' ? 'text-yellow-500' : 'text-blue-500'}`}>
                                        <option value="חדש">חדש</option><option value="בטיפול">בטיפול</option><option value="טופל">טופל / נמכר</option><option value="לא רלוונטי">לא רלוונטי</option>
                                      </select>
                                    </td>
                                    <td className="px-3 py-3">
                                      <input type="text" defaultValue={lead.notes || ''} onBlur={(e) => handleLeadUpdate(lead.id, 'notes', e.target.value)} placeholder="הוסף הערה..." className="bg-neutral-900 border border-neutral-700 text-white text-xs rounded-lg px-2 py-1.5 w-32 focus:border-blue-500 outline-none"/>
                                    </td>
                      ...
                      {leads.length === 0 && !leadsLoading && <div className="py-12 text-center text-neutral-500 text-sm">אין לידים עדיין. המערכת מוכנה!</div>}
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-[150] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/85 backdrop-blur-sm">
          <div className="bg-neutral-900 border border-neutral-800 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-3xl overflow-hidden flex flex-col shadow-2xl" style={{maxHeight:'94svh'}}>
            <div className="flex justify-between items-center px-5 py-4 border-b border-neutral-800 bg-neutral-950 flex-row-reverse shrink-0">
              ...
              <div className="text-right"><h2 className="text-lg font-bold text-white">עריכת רכב</h2><p className="text-xs text-neutral-500">{editCar.make} {editCar.model} ({editCar.year})</p></div>
            </div>
            <div className="flex-1 overflow-y-auto p-4 modal-safe-bottom">
              <form onSubmit={handleEditSave} className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                {editCar.image&&(<div className="col-span-2 md:col-span-3 mb-2"><div className="relative w-full rounded-xl overflow-hidden border border-neutral-700" style={{height:'120px'}}><img src={editCar.image} className="w-full h-full object-cover" onError={e=>e.target.src='/back.jpg'}/><div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-3"><span className="text-xs text-white/70">תמונה ראשית נוכחית</span></div></div></div>)}
                <select required value={editCar.make} onChange={e=>setEditCar({...editCar,make:e.target.value})} className="bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-blue-500 outline-none col-span-2 md:col-span-1"><option value="">יצרן</option>{ISRAELI_CAR_MAKES.map(m=><option key={m} value={m}>{m}</option>)}</select>
                {[ {ph:'דגם',k:'model',req:true},{ph:'תת דגם',k:'subModel'},{ph:'שנתון',k:'year',t:'number',req:true}, {ph:"קילומטראז'",k:'mileage'},{ph:'יד הרכב',k:'owners',t:'number'},{ph:'נפח מנוע סמ"ק',k:'engineCapacity'}, {ph:'מחיר ₪',k:'price',req:true},{ph:'החזר חודשי ₪',k:'monthlyPayment'}, ].map(f=>(<input key={f.k} required={!!f.req} type={f.t||'text'} placeholder={f.ph} value={editCar[f.k]||''} onChange={e=>setEditCar({...editCar,[f.k]:e.target.value})} className="bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-blue-500 outline-none"/>))}
                <select value={editCar.condition} onChange={e=>setEditCar({...editCar,condition:e.target.value})} className="bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-blue-500 outline-none"><option value="משומש">משומש</option><option value="חדש">חדש</option></select>
                <select value={editCar.engineType} onChange={e=>setEditCar({...editCar,engineType:e.target.value})} className="bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-blue-500 outline-none">{['בנזין','הייבריד','חשמלי','דיזל','פלאג אין הייבריד'].map(o=><option key={o} value={o}>{o}</option>)}</select>
                <select value={editCar.type||'משפחתי'} onChange={e=>setEditCar({...editCar,type:e.target.value})} className="bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-blue-500 outline-none">{['משפחתי','יוקרה','ספורט','גיפ',"7 מקומות","מיני","מנהלים"].map(o=><option key={o} value={o}>{o}</option>)}</select>
                ...
                <div className="col-span-2 md:col-span-3 flex flex-wrap items-center gap-3 flex-row-reverse bg-neutral-800/50 p-2 rounded-xl border border-neutral-700">
                  <div className="flex-1 flex items-center gap-2 flex-row-reverse"><input type="text" placeholder="מחיר מחירון" value={editCar.listPrice||''} onChange={e=>setEditCar({...editCar,listPrice:e.target.value})} disabled={!editCar.showListPrice} className="flex-1 bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2.5 text-white disabled:opacity-40 text-right outline-none"/><label className="flex items-center gap-1.5 text-xs text-neutral-400 cursor-pointer whitespace-nowrap flex-row-reverse min-h-0"><input type="checkbox" checked={!!editCar.showListPrice} onChange={e=>setEditCar({...editCar,showListPrice:e.target.checked})} className="accent-red-600 w-4 h-4"/>הצג מחירון</label></div>
                  <label className="flex items-center gap-2 text-sm text-white font-bold cursor-pointer whitespace-nowrap flex-row-reverse bg-blue-600/20 px-4 py-2.5 rounded-lg border border-blue-600/40 hover:bg-blue-600/30 transition-colors min-h-0"><input type="checkbox" checked={!!editCar.showOnHome} onChange={e=>setEditCar({...editCar,showOnHome:e.target.checked})} className="accent-blue-600 w-4 h-4"/>הצג בדף הבית</label>
                  <label className="flex items-center gap-2 text-sm text-white font-bold cursor-pointer whitespace-nowrap flex-row-reverse bg-blue-600/20 px-4 py-2.5 rounded-lg border border-blue-600/40 hover:bg-blue-600/30 transition-colors min-h-0"><input type="checkbox" checked={!!editCar.officialWarranty} onChange={e=>setEditCar({...editCar,officialWarranty:e.target.checked})} className="accent-blue-600 w-4 h-4"/>אחריות יבואן רשמי</label>
                </div>
                <div className="col-span-2 md:col-span-3"><label className="block text-xs text-neutral-400 mb-1.5 text-right">צבעים זמינים (מופרדים בפסיק. למשל: לבן, שחור, כסף)</label><input type="text" placeholder="הזן צבעים..." value={editCar.colors || ''} onChange={e=>setEditCar({...editCar, colors: e.target.value})} className="w-full bg-neutral-800 border border-neutral-700 rounded-lg px-3 py-2.5 text-white text-right focus:border-blue-500 outline-none"/></div>
                <div className="col-span-2 md:col-span-3 bg-neutral-800/50 p-4 rounded-xl border border-neutral-700">
                  <div className="flex justify-between items-center mb-3 flex-row-reverse"><label className="text-sm font-bold text-white">רמות גימור / דגמים נוספים</label><button type="button" onClick={() => setEditCar({...editCar, trims: [...(editCar.trims || []), {name: '', price: ''}]})} className="text-xs bg-neutral-900 hover:bg-blue-600 text-white px-3 py-2 rounded-lg transition-colors shadow-sm font-medium">+ הוסף דגם</button></div>
