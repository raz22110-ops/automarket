...
const ISRAELI_CAR_MAKES = [
  ...
  "במוו", "בי-ווי-די (BYD)", "ג'יפ", "ג'נסיס", "גרייטוול", "דאצ'יה", "דודג'", "הונדה", 
  "יונדאי", "וולוו", "טויוטה", "טסלה", "יגואר", "לאדה", "לקסוס", "מאזדה", "מזראטי", 
  "מיני", "מיצובישי", "מרצדס", "ניסאן", "סאנגיונג", "סובארו", "סוזוקי", "סיאט", 
  "סיטרואן", "סמארט", "סקודה", "פולקסווגן", "פורד", "פורשה", "פיאט", "פיג'ו", 
...
const CarDealershipApp = () => {
  ...
  // --- משיכת נתונים מ-Supabase בעליית האתר ---
  useEffect(() => {
    fetchInventory();
  }, []);
  ...
  const [newCar, setNewCar] = useState({
    make: '', model: '', subModel: '', year: '', condition: 'משומש', engineType: 'בנזין', mileage: '', owners: '', engineCapacity: '', price: '', monthlyPayment: '', listPrice: '', showListPrice: false, images: [], type: 'משפחתי'
  });

  // פונקציה חכמה לכיווץ תמונות לאיכות HD לפני העלאה
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          resolve(canvas.toDataURL('image/jpeg', 0.85)); // שומר באיכות HD
        };
  ...
  const handleImageUpload = async (e) => {
    ...
    if (files.length > 10) {
      alert('ניתן להעלות עד 10 תמונות לכל רכב.');
      return;
    }
    try {
  ...
  const handleAddCar = async (e) => {
    ...
    const carToAdd = {
      ...newCar,
      id: Date.now().toString(), // Supabase צריך String בתור ID
      image: mainImage,
    ...
    try {
      ...
      alert('שגיאה בשמירת הרכב למסד הנתונים. האם יצרת את הטבלה ב-Supabase?');
    }
  };
        ...
        cars.map(car => (
          <div key={car.id} className="group bg-neutral-900 rounded-2xl overflow-hidden border border-neutral-800 hover:border-red-600/50 transition-all duration-300">
            <div className="relative h-64 overflow-hidden bg-neutral-800 flex items-center justify-center">
              <div className="absolute top-4 right-4 z-10 bg-black/60 backdrop-blur-md text-white px-3 py-1 rounded-full text-sm font-medium border border-white/10">
                {car.type}
              </div>
              <div className={`absolute top-4 left-4 z-10 backdrop-blur-md px-3 py-1 rounded-full text-sm font-bold border ${car.condition === 'חדש' ? 'bg-red-600/90 text-white border-red-500' : 'bg-neutral-600/80 text-white border-neutral-500'}`}>
                {car.condition}
              </div>
              <img 
                src={car.image || '/back.jpg'} 
            ...
            <div className="p-6 text-right">
              <div className="flex justify-between items-start mb-4 flex-row-reverse">
                ...
                <div className="text-left flex flex-col items-start">
                  ...
                  {car.monthlyPayment && (
                     <p className="text-xs text-neutral-400 font-medium mt-1">החל מ- ₪ {car.monthlyPayment} לחודש</p>
                  )}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-y-3 gap-x-2 mb-6 pt-4 border-t border-neutral-800">
                <div className="flex items-center justify-end gap-2 text-sm text-neutral-300" title="קילומטראז'">
                  {car.mileage} ק"מ
                  <Gauge className="w-4 h-4 text-neutral-500" />
                </div>
                <div className="flex items-center justify-end gap-2 text-sm text-neutral-300" title="יד הרכב">
                  יד {car.owners}
                  <User className="w-4 h-4 text-neutral-500" />
                </div>
                <div className="flex items-center justify-end gap-2 text-sm text-neutral-300" title="נפח מנוע">
                  {car.engineCapacity} סמ"ק
                  <Settings className="w-4 h-4 text-neutral-500" />
                </div>
                <div className="flex items-center justify-end gap-2 text-sm text-neutral-300" title="סוג הנעה">
                  {car.engineType}
              ...
              <button 
                ...
                className="w-full bg-neutral-800 hover:bg-red-600 hover:text-white text-white py-3 rounded-xl font-bold transition-colors"
              >
                לפרטים נוספים
              </button>
            </div>
    ...
    return (
      <div className="pt-32 pb-16 bg-neutral-950 min-h-screen text-right" dir="rtl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <button 
            onClick={onBack}
            className="flex items-center gap-2 text-neutral-400 hover:text-red-500 transition-colors mb-8 font-medium bg-neutral-900/50 px-4 py-2 rounded-full w-fit border border-neutral-800 mr-0 ml-auto"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-8">
              <div className="bg-neutral-900 rounded-3xl overflow-hidden border border-neutral-800 h-[300px] md:h-[500px] relative">
                <div className={`absolute top-6 right-6 z-10 backdrop-blur-md px-4 py-2 rounded-full text-sm font-bold border ${car.condition === 'חדש' ? 'bg-red-600/90 text-white border-red-500' : 'bg-neutral-600/80 text-white border-neutral-500'}`}>
                  {car.condition}
                </div>
                <img 
                  src={(car.images && car.images.length > 0) ? car.images[activeImageIndex] : (car.image || '/back.jpg')} 
                  alt={`${car.make} ${car.model}`} 
                ...
                <div className="flex gap-4 overflow-x-auto pb-2 flex-row-reverse">
                  {car.images.map((img, idx) => (
                    <div 
                      ...
                      className={`shrink-0 w-24 h-16 rounded-lg overflow-hidden cursor-pointer border-2 transition-all ${activeImageIndex === idx ? 'border-red-600 opacity-100' : 'border-transparent opacity-50 hover:opacity-100'}`}
                    >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-neutral-900 p-6 rounded-2xl border border-neutral-800 text-center">
                  ...
                  <p className="text-neutral-400 text-sm mb-1">שנת ייצור</p>
                  <p className="text-white font-bold text-lg">{car.year}</p>
                </div>
                <div className="bg-neutral-900 p-6 rounded-2xl border border-neutral-800 text-center">
                  <Settings className="w-8 h-8 text-red-600 mx-auto mb-3" />
                  <p className="text-neutral-400 text-sm mb-1">סוג הנעה</p>
                  <p className="text-white font-bold text-lg">{car.engineType}</p>
                </div>
                <div className="bg-neutral-900 p-6 rounded-2xl border border-neutral-800 text-center">
                  <Gauge className="w-8 h-8 text-red-600 mx-auto mb-3" />
                  <p className="text-neutral-400 text-sm mb-1">קילומטראז'</p>
                  <p className="text-white font-bold text-lg" dir="ltr">{car.mileage} km</p>
                </div>
                <div className="bg-neutral-900 p-6 rounded-2xl border border-neutral-800 text-center">
                  <Car className="w-8 h-8 text-red-600 mx-auto mb-3" />
                  <p className="text-neutral-400 text-sm mb-1">קטגוריה</p>
                  <p className="text-white font-bold text-lg">{car.type}</p>
                </div>
              </div>

              <div className="flex gap-4">
                 <div className="flex-1 bg-neutral-900 p-4 rounded-2xl border border-neutral-800 text-center">
                    <p className="text-neutral-400 text-sm mb-1">יד הרכב</p>
                    <p className="text-white font-bold text-lg">{car.owners}</p>
                 </div>
                 <div className="flex-1 bg-neutral-900 p-4 rounded-2xl border border-neutral-800 text-center">
                    <p className="text-neutral-400 text-sm mb-1">נפח מנוע (סמ"ק)</p>
                    <p className="text-white font-bold text-lg">{car.engineCapacity}</p>
                 </div>
              </div>
              ...
              <div className="bg-neutral-900 p-8 rounded-2xl border border-neutral-800">
                ...
                <ul className="space-y-4 text-right">
                  <li className="flex items-start gap-3 flex-row-reverse">
                    ...
                    <span className="text-neutral-300">אחריות מקיפה על הרכב, מנוע וגיר לשקט הנפשי שלך.</span>
                  </li>
                  <li className="flex items-start gap-3 flex-row-reverse">
                    <div className="mt-1 bg-red-600/20 p-1 rounded-full"><Check className="w-4 h-4 text-red-500" /></div>
                    <span className="text-neutral-300">אפשרות ל-100% מימון בתנאים המשתלמים ביותר בשוק.</span>
                  </li>
                  <li className="flex items-start gap-3 flex-row-reverse">
                    <div className="mt-1 bg-red-600/20 p-1 rounded-full"><Check className="w-4 h-4 text-red-500" /></div>
                    <span className="text-neutral-300">אפשרות לטרייד-אין עתידי בהתחייבות.</span>
                  </li>
                  <li className="flex items-start gap-3 flex-row-reverse">
                    <div className="mt-1 bg-red-600/20 p-1 rounded-full"><Check className="w-4 h-4 text-red-500" /></div>
                    <span className="text-neutral-300">רכב שעבר בדיקה קפדנית והכנה מלאה לפני מסירה.</span>
                  </li>
                </ul>
              </div>
            </div>
              ...
              <div className="bg-neutral-900 p-8 rounded-2xl border border-neutral-800 relative overflow-hidden">
                ...
                <div className="relative z-10 text-right">
                  ...
                  <div className="border-t border-neutral-800 pt-6">
                    {car.showListPrice && car.listPrice && (
                      <div className="mb-1">
                        <span className="text-neutral-500 text-sm">מחיר מחירון: </span>
                        <span className="text-neutral-500 line-through font-medium">₪ {car.listPrice}</span>
                      </div>
                    )}
                    <div className="flex flex-col gap-2">
                      <span className="text-4xl font-bold text-red-600">₪ {car.price}</span>
                      {car.monthlyPayment && (
                         <span className="text-sm font-medium text-neutral-400">החזר חודשי החל מ- ₪ {car.monthlyPayment}</span>
                      )}
                    </div>
                  </div>
                </div>
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans" dir="rtl">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="hidden lg:flex space-x-8 space-x-reverse items-center">
              ...
              <button onClick={() => setIsTradeInOpen(true)} className="text-red-500 font-bold hover:text-red-400 transition-colors mr-4">
                הצעת טרייד אין
              </button>
              <button onClick={() => navigateTo('home', 'contact')} className="bg-red-600 text-white px-6 py-2 rounded-full font-bold hover:bg-red-500 transition-colors">
                צור קשר
              </button>
            </div>
      ...
      {selectedCar ? (
        ...
        <GenericInventoryPage 
          cars={inventory.filter(car => car.condition === 'חדש')} 
          title={<>מלאי <span className="text-red-600">רכבים חדשים</span></>}
          subtitle={`מציג ${inventory.filter(car => car.condition === 'חדש').length} רכבים מתוך המלאי שלנו. כל רכב עבר בדיקה קפדנית.`}
        />
      ) : currentView === 'used' ? (
        <GenericInventoryPage 
          cars={inventory.filter(car => car.condition === 'משומש')} 
          title={<>מלאי <span className="text-red-600">רכבים משומשים</span></>}
          subtitle={`מציג ${inventory.filter(car => car.condition === 'משומש').length} רכבים מתוך המלאי שלנו. כל רכב עבר בדיקה קפדנית.`}
        />
      ) : currentView === 'search' ? (
        <GenericInventoryPage 
          cars={searchResults} 
          title={<span className="text-red-600">{searchTitle}</span>}
          subtitle={searchSubtitle}
        />
      ) : (
        <>
          <div className="relative pt-20 pb-20 min-h-[90vh] flex items-center justify-center">
            ...
            <div className="relative z-10 text-center px-4 max-w-5xl mx-auto mt-20 w-full">
              ...
              <div className="bg-neutral-900/90 backdrop-blur-xl rounded-2xl border border-neutral-800 shadow-2xl overflow-hidden mt-8 text-right">
                ...
                <div className="p-6 text-right">
                    ...
                    <div className="space-y-6 text-right">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <select 
                          ...
                          className="bg-neutral-800 text-white rounded-xl px-4 py-3 border border-neutral-700 focus:outline-none focus:border-red-600 text-right appearance-none font-medium"
                        >
                          <option value="">כל היצרנים</option>
                          {ISRAELI_CAR_MAKES.map(make => (
                            <option key={make} value={make}>{make}</option>
                          ))}
                        </select>

                        <select 
                          value={searchCondition} onChange={e => setSearchCondition(e.target.value)}
                          className="bg-neutral-800 text-white rounded-xl px-4 py-3 border border-neutral-700 focus:outline-none focus:border-red-600 text-right appearance-none font-medium"
                        >
                          <option value="">כל המצבים (חדש/משומש)</option>
                      ...
                      <div className="bg-neutral-950 p-6 rounded-2xl border border-neutral-800">
                        <div className="flex justify-between text-sm mb-4 items-end flex-row-reverse">
                          ...
                          <span className="text-white font-bold text-2xl">עד ₪ {searchBudget.toLocaleString()}</span>
                        </div>
                        <input 
                          type="range" min="10000" max="1500000" step="10000" 
                          value={searchBudget} onChange={(e) => setSearchBudget(Number(e.target.value))} 
                  ...
                  {searchTab === 'finance' && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center text-right">
                      <div className="md:col-span-2 space-y-6">
                         <div className="flex gap-2 p-1 bg-neutral-950 rounded-lg border border-neutral-800 flex-row-reverse">
                           <button onClick={() => setFinanceCondition('new')} className={`flex-1 py-2 rounded-md font-bold text-sm md:text-base transition-colors ${financeCondition === 'new' ? 'bg-red-600 text-white shadow-md' : 'text-neutral-400 hover:text-white'}`}>רכב חדש (ריבית 4.5%)</button>
                           <button onClick={() => setFinanceCondition('used')} className={`flex-1 py-2 rounded-md font-bold text-sm md:text-base transition-colors ${financeCondition === 'used' ? 'bg-red-600 text-white shadow-md' : 'text-neutral-400 hover:text-white'}`}>רכב משומש (ריבית 6.1%)</button>
                         </div>
                         <div>
                           <div className="flex justify-between text-sm mb-2 items-end flex-row-reverse"><span className="text-neutral-400 font-medium">שווי הרכב</span><span className="text-white font-bold text-xl">₪ {financePrice.toLocaleString()}</span></div>
                         ...
                         <div>
                           <div className="flex justify-between text-sm mb-2 items-end flex-row-reverse"><span className="text-neutral-400 font-medium">מקדמה</span><span className="text-white font-bold text-xl">₪ {financeDownPayment.toLocaleString()}</span></div>
                         ...
                         <div>
                           <div className="flex justify-between text-sm mb-2 items-end flex-row-reverse"><span className="text-neutral-400 font-medium">מספר תשלומים</span><span className="text-white font-bold text-xl">{financePayments} חודשים</span></div>
                      ...
                      <div className="bg-neutral-950 p-6 rounded-2xl border border-red-600/30 flex flex-col items-center justify-center text-center shadow-[0_0_20px_rgba(220,38,38,0.1)] h-full">
                         <span className="text-neutral-400 text-sm mb-2 font-medium">החזר חודשי משוער</span>
                         <span className="text-5xl font-black text-red-600 mb-2">₪ {calculateMonthly().toLocaleString()}</span>
                         <span className="text-xs text-neutral-500 mb-6 px-4">*החישוב הינו משוער וכפוף לאישור הגוף המממן. ט.ל.ח</span>
                         <button onClick={handleFinanceSearch} className="w-full bg-red-600 hover:bg-red-500 text-white font-bold py-4 rounded-xl transition-colors shadow-lg flex items-center justify-center gap-2 flex-row-reverse">
          ...
          <section id="services" className="py-24 bg-neutral-900 border-y border-neutral-800">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="bg-neutral-950 p-8 rounded-2xl border border-neutral-800 text-center hover:-translate-y-2 transition-transform duration-300">
                  <div className="w-16 h-16 bg-neutral-900 rounded-full flex items-center justify-center mx-auto mb-6 border border-neutral-700 text-red-600">
                    <DollarSign className="w-8 h-8" />
                  </div>
                  <h3 className="text-xl font-bold mb-3">פתרונות מימון</h3>
                  <p className="text-neutral-400">אפשרויות מימון גמישות עד 100% ופריסת תשלומים נוחה מול הגופים המובילים.</p>
                </div>
                <div className="bg-neutral-950 p-8 rounded-2xl border border-neutral-800 text-center hover:-translate-y-2 transition-transform duration-300">
                  <div className="w-16 h-16 bg-neutral-900 rounded-full flex items-center justify-center mx-auto mb-6 border border-neutral-700 text-red-600">
                    <Car className="w-8 h-8" />
                  </div>
                  <h3 className="text-xl font-bold mb-3">טרייד אין הוגן</h3>
                  <p className="text-neutral-400">החלף את הרכב הישן בחדש או משומש בתנאים מועדפים, שקיפות מלאה ומחירון הוגן.</p>
                </div>
                <div className="bg-neutral-950 p-8 rounded-2xl border border-neutral-800 text-center hover:-translate-y-2 transition-transform duration-300">
                  <div className="w-16 h-16 bg-neutral-900 rounded-full flex items-center justify-center mx-auto mb-6 border border-neutral-700 text-red-600">
                    <Shield className="w-8 h-8" />
                  </div>
                  <h3 className="text-xl font-bold mb-3">אחריות ובדיקה</h3>
                  <p className="text-neutral-400">כל רכב עובר בדיקות מקיפות ונמסר עם שקיפות מלאה לגבי מצבו לשקט הנפשי שלך.</p>
                </div>
              </div>
            </div>
          </section>
              ...
              <div className="flex flex-col md:flex-row gap-12 items-center flex-row-reverse">
                <div className="md:w-1/2 text-right">
                  ...
                  <p className="text-neutral-300 text-lg mb-6 leading-relaxed">
                    באוטו מרקט, אנחנו מאמינים שקניית רכב צריכה להיות חוויה מרגשת, בטוחה ונטולת דאגות. עם שנים של ניסיון בענף הרכב, בנינו סוכנות ששמה את הלקוח במרכז, עם דגש על שקיפות מוחלטת, מקצועיות ושירות VIP.
                  </p>
                  <ul className="space-y-4 mb-8">
                    <li className="flex items-start gap-3 flex-row-reverse text-right">
                      ...
                      <span className="text-neutral-300"><strong>מבחר מוקפד:</strong> כל רכב באולם התצוגה שלנו נבחר בקפידה ועובר סדרת בדיקות קפדניות.</span>
                    </li>
                    <li className="flex items-start gap-3 flex-row-reverse text-right">
                      <div className="mt-1 bg-red-600/20 p-1 rounded-full shrink-0"><Check className="w-4 h-4 text-red-500" /></div>
                      <span className="text-neutral-300"><strong>שקיפות מלאה:</strong> אנחנו מציגים בפניך את כל ההיסטוריה של הרכב, ללא הפתעות.</span>
                    </li>
                  </ul>
                  <button onClick={() => navigateTo('home', 'contact')} className="bg-transparent border-2 border-red-600 text-red-500 px-8 py-3 rounded-xl font-bold hover:bg-red-600 hover:text-white transition-colors">
                    דברו איתנו
                  </button>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col items-center justify-center text-center mb-16">
            ...
            <p className="text-neutral-400 max-w-2xl text-lg">
              סוכנות הרכב המובילה בישראל לכל סוגי הרכבים. חווית רכישה, שירות, מימון וטרייד-אין תחת קורת גג אחת בסטנדרט שטרם הכרתם.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12 border-t border-neutral-800/50 pt-16">
            ...
            <div className="text-right">
              ...
              <ul className="space-y-6 text-neutral-400">
                <li className="flex items-center gap-4 flex-row-reverse">
                  <div className="bg-neutral-900 p-3 rounded-xl text-red-600 shadow-inner">
            ...
            <div className="text-right md:text-center flex flex-col items-end md:items-center">
              ...
              <ul className="space-y-4 text-neutral-400 inline-block text-right">
                <li><button onClick={() => navigateTo('new')} className="hover:text-red-600 transition-colors flex items-center gap-2 flex-row-reverse"><ChevronRight className="w-4 h-4 text-red-600 rotate-180" /> מלאי רכבים חדשים</button></li>
                <li><button onClick={() => navigateTo('used')} className="hover:text-red-600 transition-colors flex items-center gap-2 flex-row-reverse"><ChevronRight className="w-4 h-4 text-red-600 rotate-180" /> רכבי יד שנייה</button></li>
                <li><button onClick={() => setIsTradeInOpen(true)} className="hover:text-red-600 transition-colors flex items-center gap-2 flex-row-reverse"><ChevronRight className="w-4 h-4 text-red-600 rotate-180" /> טופס הצעת טרייד-אין</button></li>
                <li><button onClick={() => navigateTo('home', 'about')} className="hover:text-red-600 transition-colors flex items-center gap-2 flex-row-reverse"><ChevronRight className="w-4 h-4 text-red-600 rotate-180" /> אודות הסוכנות</button></li>
              </ul>
            </div>

            <div className="text-right">
              <h4 className="text-white font-bold mb-8 text-xl">שעות פעילות ורשתות</h4>
              <ul className="space-y-3 text-neutral-400 mb-8">
                <li className="flex justify-between items-center border-b border-neutral-800/50 pb-2 flex-row-reverse">
                  <span className="text-white">א' - ה'</span>
                  <span className="font-medium">09:00 - 18:00</span>
                </li>
                <li className="flex justify-between items-center border-b border-neutral-800/50 pb-2 flex-row-reverse">
                  <span className="text-white">שישי</span>
                  <span className="font-medium">09:00 - 13:00</span>
                </li>
                <li className="flex justify-between items-center flex-row-reverse">
      ...
      <a href="https://wa.me/972526441855?text=שלום,%20הגעתי%20מהאתר%20ואני%20מעוניין%20בפרטים%20על%20רכב" target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 bg-[#25D366] text-white p-4 rounded-full shadow-[0_4px_20px_rgba(37,211,102,0.4)] hover:scale-110 transition-all z-40 group">
      ...
      {isPasswordPromptOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-sm p-6 text-right relative shadow-[0_0_30px_rgba(220,38,38,0.1)]">
            <button onClick={() => { setIsPasswordPromptOpen(false); setPasswordError(''); setAdminPassword(''); }} className="absolute top-4 left-4 text-neutral-400 hover:text-white">
              <X className="w-5 h-5" />
            </button>
            ...
            <form onSubmit={handlePasswordSubmit}>
              <input 
                ...
                placeholder="סיסמה" 
                className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-3 text-white focus:border-red-600 outline-none text-right mb-4" 
                autoFocus
              />
            ...
            <div className="p-6 md:p-8 overflow-y-auto">
              <form onSubmit={handleTradeInSubmit} className="space-y-6 text-sm text-right">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-right">
                  <div>
                    <label className="block text-neutral-400 mb-1">שם מלא</label>
                    <input type="text" value={tradeData.name} onChange={e => setTradeData({...tradeData, name: e.target.value})} placeholder="לדוג': ישראל ישראלי" className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-3 text-white focus:border-red-600 outline-none text-right" required />
                  </div>
                  <div>
                    <label className="block text-neutral-400 mb-1">מספר טלפון</label>
                    <input type="tel" value={tradeData.phone} onChange={e => setTradeData({...tradeData, phone: e.target.value})} placeholder="לדוג': 052-1234567" className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-3 text-white focus:border-red-600 outline-none text-right" required />
                  </div>
                  <div>
                    <label className="block text-neutral-400 mb-1">יצרן</label>
                    <input type="text" value={tradeData.make} onChange={e => setTradeData({...tradeData, make: e.target.value})} placeholder="לדוג': יונדאי" className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-3 text-white focus:border-red-600 outline-none text-right" required />
                  </div>
                  <div>
                    <label className="block text-neutral-400 mb-1">תת דגם</label>
                    <input type="text" value={tradeData.model} onChange={e => setTradeData({...tradeData, model: e.target.value})} placeholder="לדוג': טוסון Elite" className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-3 text-white focus:border-red-600 outline-none text-right" required />
                  </div>
                  <div>
                    <label className="block text-neutral-400 mb-1">שנתון</label>
                    <input type="number" value={tradeData.year} onChange={e => setTradeData({...tradeData, year: e.target.value})} placeholder="לדוג': 2021" className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-3 text-white focus:border-red-600 outline-none text-right" required />
                  </div>
                  <div>
                    ...
                    <select value={tradeData.engine} onChange={e => setTradeData({...tradeData, engine: e.target.value})} className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-3 text-white focus:border-red-600 outline-none text-right" required>
                      ...
                      <option value="פלאג אין הייבריד">פלאג אין הייבריד</option>
                    </select>
                  </div>
                  <div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="bg-neutral-950 p-6 rounded-xl border border-neutral-800 mb-8">
                ...
                <form onSubmit={handleAddCar} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                  ...
                  <input required type="number" placeholder="יד הרכב (0,1..)" value={newCar.owners} onChange={e => setNewCar({...newCar, owners: e.target.value})} className="bg-neutral-900 border border-neutral-700 rounded-lg px-4 py-2 text-white text-right" />
                  ...
                  <select value={newCar.type} onChange={e => setNewCar({...newCar, type: e.target.value})} className="bg-neutral-900 border border-neutral-700 rounded-lg px-4 py-2 text-white text-right">
                    <option value="משפחתי">משפחתי</option><option value="יוקרה">יוקרה</option><option value="ספורט">ספורט</option><option value="SUV">SUV</option>
                  </select>
                  
                  <div className="md:col-span-3">
                    <label className="block text-xs text-neutral-400 mb-1">תמונות (עד 10 תמונות)</label>
                    ...
                    {newCar.images && newCar.images.length > 0 && (
                      <p className="text-xs text-green-500 mt-1 font-medium">✓ נבחרו {newCar.images.length} תמונות בהצלחה</p>
                    )}
                  </div>
              ...
              <div className="bg-neutral-950 rounded-xl border border-neutral-800 overflow-hidden overflow-x-auto">
                <table className="w-full text-right text-sm">
                  <thead className="bg-neutral-900 text-neutral-400 border-b border-neutral-800">
                    <tr><th className="p-4 text-center">פעולות</th><th className="p-4">מחיר (₪)</th><th className="p-4">מצב רכב</th><th className="p-4">יצרן ודגם</th><th className="p-4 text-right">תמונה</th></tr>
                  </thead>
